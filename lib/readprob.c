  ...
  *pn=0;
	  ...
	  if (blksz < 0)
	  ...
	  else
	    {
	       ...
	       * It's a matrix block.  In the I32LP64 model, we can't do array 
               * indexing for huge blocks using 32 bit integers.  2000000 is 
  ...
  for (i=1; i<=*pk; i++)
      ...
      while (p != NULL)
	{
	  ...
	  p->iindices=(long int *)malloc((p->numentries+1)*sizeof(long int));
	  ...
	  p->jindices=(long int *)malloc((p->numentries+1)*sizeof(long int));
