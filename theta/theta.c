     ...
     char *argv[];
{
  ...
  constraints[1].blocks->iindices=(long int *) malloc((n+1)*sizeof(long int));
  constraints[1].blocks->jindices=(long int *) malloc((n+1)*sizeof(long int));
  ...
  for (i=2; i<=m+1; i++)
	...
	malloc(sizeof(struct sparseblock));
      ...
      constraints[i].blocks->iindices=(long int *) malloc((2)*sizeof(long int));
      constraints[i].blocks->jindices=(long int *) malloc((2)*sizeof(long int));
  ...
  ret=easy_sdp(n,m+1,C,a,constraints,0.0,&X,&y,&Z,&pobj,&dobj);
